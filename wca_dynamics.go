...
func main() {
	...
	file_name := "wca_"+strconv.Itoa(len(times))+"_"+strconv.Itoa(N)+".txt"
...
func dynamics(parlist []Particle, N int, L float64) ([]float64, []float64, []float64) {
	...
	x_forces, y_forces, z_forces := force_matrices(parlist, N, L, sigma)

	// open ovito files
	periodic_file, err := os.Create("ovito_periodic.xyz")
	if err != nil {
		fmt.Println(err)
	}
	defer periodic_file.Close()
	real_file, err := os.Create("ovito_real.xyz")
	if err != nil {
		fmt.Println(err)
	}
	...
	for step := 0; step <= total_steps; step++ {
		...
		for i := 0; i < N; i++ {
			...
			parlist[i].z_pos,parlist[i].z_real = update_pos(parlist[i], z_forces, L, dt, "z", i)
		}
		
		// now update forces, will use new positions
		x_forces_new, y_forces_new, z_forces_new := force_matrices(parlist, N, L, sigma)
		...
		// write ovito files every 100 steps, checking if in "production run"
		...
		// show progress
		if math.Mod(dt*float64(step), 10.0) == 0 {
...
func calc_potential(parlist []Particle, L float64) float64{
	...
	for i := range parlist {
		for j := range parlist {
			p1 := parlist[i]
			p2 := parlist[j]
			// following conditonals set p2 positon to minimum image
			...
			if (p1.z_pos - p2.z_pos) < -L/2 {
				p2.z_pos = p2.z_pos - L
			} else if (p1.z_pos - p2.z_pos) > L/2 {
				p2.z_pos = p2.z_pos + L
			}
			radius := distance(p1,p2)
			if i != j {
				pe = pe + 4*((1/math.Pow(radius,12)) - (1/math.Pow(radius,6)) + 1/4.0)
			}
		}
	}
...
func calc_kinetic(parlist []Particle) float64{
	var ke float64
	for i := range parlist {
		ke = ke + math.Pow(parlist[i].x_v,2) + math.Pow(parlist[i].y_v,2) + math.Pow(parlist[i].z_v,2)
	}
	return .5*ke
}
...
func update_pos(curr_p Particle, forces [][]float64, L float64, dt float64, axis string, index int) (float64, float64) {
	...
	if axis == "z" {
		new_pos = curr_p.z_pos + curr_p.z_v*dt + row_sum(forces, index)*dt*dt*.5
		real_pos = curr_p.z_real + curr_p.z_v*dt + row_sum(forces, index)*dt*dt*.5
	}
...
func update_veloc(curr_p Particle, forces [][]float64, forces_new [][]float64, dt float64, axis string, index int) float64 {
	...
	if axis == "z" {
		new_veloc = curr_p.z_v + row_sum(add_matrix(forces, forces_new), index)*dt*.5
	}
	return new_veloc
}
...
func force_matrices(parlist []Particle, N int, L float64, sigma float64) ([][]float64, [][]float64, [][]float64) {
	x_forces := two_d(N)
	y_forces := two_d(N)
	z_forces := two_d(N)
	...
	for i := 0; i < N; i++ {
		for j := i + 1; j < N; j++ {
			...
			z_forces[i][j] = force_between(p1, p2, "z", L, sigma)
		}
	}
...
func force_between(p1 Particle, p2 Particle, axis string, L float64, sigma float64) float64 {
	...
	if radius >= (math.Pow(2.0,1/6.0))*sigma {
		return 0
	}
	temp_f := (48 / math.Pow(radius, 2)) * (1/(math.Pow(radius, 12)) - .5*(1/math.Pow(radius, 6)))
	var force float64
	// use right directon based on coordinate
	if axis == "x" {              
	...
	if axis == "z" {
		// must be z force
	    force = temp_f * (p1.z_pos - p2.z_pos)
	}
	return force
}
...
func distance(p1 Particle, p2 Particle) float64 {
	return math.Sqrt(math.Pow((p1.x_pos-p2.x_pos), 2) + math.Pow((p1.y_pos-p2.y_pos), 2) + math.Pow((p1.z_pos-p2.z_pos), 2))
}
...
func write_ovito(N int, parlist []Particle, file *os.File, kind string) {
	...
	if err2 != nil {
		fmt.Println("Error writing x	y	z")
	}
	// while at a current timestep, write all the positions of all the particles
	for i := 0; i < len(parlist); i ++ {
		p := parlist[i]
		// assume in "real" case, otherwise revalued for periodic values
		string_x := fmt.Sprintf("%f", p.x_real)
		string_y := fmt.Sprintf("%f", p.y_real)
		string_z := fmt.Sprintf("%f", p.z_real)
		if kind == "periodic" {
			string_x = fmt.Sprintf("%f", p.x_pos)
			string_y = fmt.Sprintf("%f", p.y_pos)
			string_z = fmt.Sprintf("%f", p.z_pos)
		}
		_,err := file.WriteString(string_x+"	"+string_y+"	"+string_z+"\n")
		if err != nil {
...
func process(particle_data []string) []Particle {
	...
	for i := 0; i <= len(particle_data)-1; i++ {
		...
		// grab float64 conversion of each value 
		parlist[i].x_pos, _ = strconv.ParseFloat(tempdata[0], 64)
		parlist[i].y_pos, _ = strconv.ParseFloat(tempdata[1], 64)
		parlist[i].z_pos, _ = strconv.ParseFloat(tempdata[2], 64)
		parlist[i].x_real = parlist[i].x_pos
		parlist[i].y_real = parlist[i].y_pos
		parlist[i].z_real = parlist[i].z_pos
		parlist[i].x_v, _ = strconv.ParseFloat(tempdata[3], 64)
		parlist[i].y_v, _ = strconv.ParseFloat(tempdata[4], 64)
		parlist[i].z_v, _ = strconv.ParseFloat(tempdata[5], 64)
	}
	return parlist
}
