...
func main() {
	...
	// write pe, ke, and times to file
	file_name := "01_100000_11_"+strconv.Itoa(N)+".txt"
...
func dynamics(parlist []Particle, N int, L float64) ([]float64, []float64, []float64) {
	...
	// open ovito file
	file, err := os.Create("ovito_d11_100particle.xyz")
	if err != nil {
		fmt.Println(err)
	}
	defer file.Close()
	
	// write intial positons
	write_ovito(N, parlist, file)

	// now do main loop, stepping dynamics until time is up
	for step := 0; step <= total_steps; step++ {
		// for each particle, update x and y positions
		for i := 0; i < N; i++ {
			parlist[i].x_pos = update_pos(parlist[i], x_forces, L, dt, "x", i)
			parlist[i].y_pos = update_pos(parlist[i], y_forces, L, dt, "y", i)
		}
		...
		// show progress
		if math.Mod(dt*float64(step), 100.0) == 0 {
...
func update_pos(curr_p Particle, forces [][]float64, L float64, dt float64, axis string, index int) float64 {
	var new_pos float64
	if axis == "x" {
	...
	// return periodic modulo of position
	return math.Mod((2*L)+new_pos, L)
}
...
func write_ovito(N int, parlist []Particle, file *os.File) {
	string_N := strconv.Itoa(N)
	_,err := file.WriteString(string_N+"\n")
	_,err2 := file.WriteString("x	y"+"\n")
	if err != nil {
		fmt.Println("Error writing num")
	}
	if err2 != nil {
		fmt.Println("Error writing xyz")
	}
